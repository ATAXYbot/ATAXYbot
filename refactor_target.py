...
def refactor():
    with open('temp.jsx', 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Add showTargetPortal state
    state_pattern = r"const \[targetsData, setTargetsData\] = useState\(\(\) => safeGetJSON\('ataxy_targets', null\) \|\| \{\}\);"
    state_replacement = "const [targetsData, setTargetsData] = useState(() => safeGetJSON('ataxy_targets', null) || {});\n            const [showTargetPortal, setShowTargetPortal] = useState(false);\n            const [targetPortalTab, setTargetPortalTab] = useState('present');"
    if state_pattern in content:
        content = content.replace(state_pattern, state_replacement)
    else:
        print("Could not find targetsData state declaration")

    # 2. Add 'Set Target' button
    greeting_pattern = r"Ready to crack your goals\?</p>\s*</div>"
    greeting_replacement = """Ready to crack your goals?</p>
    ...
    date_picker_pattern = r'<input type="date" min="2026-01-01" max="2126-12-31" value=\{selectedDate\} onChange=\{\(e\) => setSelectedDate\(e\.target\.value\)\} className="[^"]+" />'
    content = re.sub(date_picker_pattern, ' <span className="text-xs font-bold text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-md">Today</span>', content)

    # Remove the form completely
    form_pattern = r'<form onSubmit=\{addTarget\} className="flex flex-col sm:flex-row gap-2 border-t border-gray-100 dark:border-gray-800 pt-4 mt-2">.*?</form>'
            ...
            const TargetPortalOverlay = () => {
                ...
                return (
                    <div className="fixed inset-0 z-[3000000] bg-gray-50/95 dark:bg-gray-950/95 backdrop-blur-xl animate-in fade-in zoom-in-95 flex flex-col">
                        <div className="flex-none pt-safe-top bg-white dark:bg-gray-900 shadow-sm border-b border-gray-200 dark:border-gray-800">
                        ...
                        <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-gray-50 via-gray-50 to-transparent dark:from-gray-950 dark:via-gray-950 pt-10 pb-[max(1rem,env(safe-area-inset-bottom))]">
                            <form onSubmit={(e) => {
                                e.preventDefault();
...
"""
    main_return_pattern = r"return \(\s*<VoiceRoomErrorBoundary>"
    if main_return_pattern in content or "return (\n                <VoiceRoomErrorBoundary>" in content:
        content = re.sub(r"(return \(\s*<VoiceRoomErrorBoundary>)", portal_component + "\n" + r"\1", content)
    else:
        print("Could not find main return")
        
    render_portal_pattern = r"(\{showSplash && <GlobalLoader)"
    content = re.sub(render_portal_pattern, r"<TargetPortalOverlay />\n                        \1", content)

    with open('temp_updated.jsx', 'w', encoding='utf-8') as f:
        f.write(content)
        
    print("Refactored temp_updated.jsx successfully!")
